...
function Employee(name, email, hireDate) {
  ...
  */
  ...
  /*
    Here we have a constructor function named Movie that takes in 3 parameters:
      name (a string), genre (a string), and rating (a number withing 1-100 that has been averaged out 
      from previous given ratings).
  ...
  */
  ...
  //Name this method addSavedPost. It should take in three parameters: id (a number), title (a string) and 
  //rating (a number). Use these parameters to create a new object and add it to the savedPosts array. 
  ...
  User.prototype.removeSavedPost =function(num){
    this.savedPosts = this.savedPosts.filter(function(obj){
      return obj.id !== num;
      
    })
    //this.savedPosts = this.savedPosts.filter((obj) => obj.id !== num) Same thing^^

  }
  ////////// PROBLEM 7 //////////
  
  // You will continue to use the constructor function you created in problem 5.
  // Write a prototype method for the User constructor function named changePostRating that will take in 
  //two number parameters. The first will be an id (a number) and the second will be the new rating (a number). 
  //Use the id to find the matching object in the savedPosts array. Once you find the matching object, update 
  //it's rating score with the new rating parameter.
  
  // Code here
  User.prototype.changePostRating = function(id, newRating){
    let rating = this.savedPosts.find(function(obj){
      return obj.id === id;
    })
